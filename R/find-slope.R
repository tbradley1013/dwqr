...
#' @param chlorine_col the unquoted name of the chlorine residual column
#' @param group_col the unqouted name of a grouping column, if grouping is
#' needed - e.g. if the dataset contains multiple sites.
#' @param rolling_window how large should the rolling mean window be?
#' @importFrom stats smooth.spline predict
find_slope <- function(data, date_col, chlorine_col, group_col = NULL, rolling_window = 8){
  if (!"data.frame" %in% class(data)) stop("data must be a data.frame or a tibble")

  date_col <- rlang::enquo(date_col)
  date_name <- rlang::quo_name(date_col)
  chlorine_col <- rlang::enquo(chlorine_col)
  chlorine_name <- rlang::quo_name(chlorine_col)
  ...
  output <- data %>%
    dplyr::filter(!is.na(!!chlorine_col),
                  !is.na(!!date_col)) %>%
    tidyr::nest() %>%
    dplyr::mutate(
      spline = purrr::map(.data$data, ~smooth.spline(.x[[ !!date_name ]], .x[[ !!chlorine_name ]])),
      ...
      data_new = purrr::map2(.data$data, .data$first_deriv, ~{
        .x %>%
          ...
          dplyr::mutate(
            roll_mean = rolling_mean(!!chlorine_col)
          ) %>%
